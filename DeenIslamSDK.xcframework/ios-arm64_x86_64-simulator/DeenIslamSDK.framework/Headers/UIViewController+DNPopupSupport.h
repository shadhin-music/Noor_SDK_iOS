//
//  UIViewController+DNPopupSupport.h
//  DNPopupController
//
...
extern const double DNSnapPercentDefault NS_REFINED_FOR_SWIFT;

/// Available interaction styles with the popup bar and popup content view.
typedef NS_ENUM(NSInteger, DNPopupInteractionStyle) {
	/// The default interaction style for the current environment.
	DNPopupInteractionStyleDefault,
	
	/// Drag interaction style.
	DNPopupInteractionStyleDrag,
	
	/// Snap interaction style.
	DNPopupInteractionStyleSnap,
	
	/// Scroll interaction style.
	DNPopupInteractionStyleScroll,
	
	/// No interaction
	DNPopupInteractionStyleNone = 0xFFFF
} NS_REFINED_FOR_SWIFT NS_SWIFT_NAME(UIViewController.__PopupInteractionStyle);

/// The state of the popup presentation.
typedef NS_ENUM(NSInteger, DNPopupPresentationState){
	/// The popup bar is hidden and no presentation is taking place.
	DNPopupPresentationStateBarHidden = 0,
	
	/// The popup bar is presented and is closed.
	DNPopupPresentationStateBarPresented = 1,
	
	/// The popup is open and the content controller's view is displayed.
	DNPopupPresentationStateOpen = 3,
	
	DNPopupPresentationStateHidden DN_DEPRECATED_API("Use DNPopupPresentationStateBarHidden instead.") = DNPopupPresentationStateBarHidden,
	DNPopupPresentationStateClosed DN_DEPRECATED_API("Use DNPopupPresentationStateBarPresented instead.") = DNPopupPresentationStateBarPresented,
	DNPopupPresentationStateTransitioning DN_UNAVAILABLE_API("Should no longer be used.") = 2,
} NS_SWIFT_NAME(UIViewController.PopupPresentationState);

/// Popup content support for ``UIViewController`` subclasses.
@interface UIViewController (DNPopupContent)

/// The popup item used to represent the view controller in a popup presentation. (read-only)
///
/// This is a unique instance of ``DNPopupItem``, created to represent the view controller when it is presented in a popup. The ``DNPopupItem`` object is created the first time the property is accessed. Therefore, you should not access this property if you are not using popup presentation to display the view controller. To ensure the popup item is configured, you can either override this property and add code to create the bar button items when first accessed or create the items in your view controller's initialization code.
///
/// The default behavior is to create a popup item that displays the view controller's title.
@property (nonatomic, retain, readonly) DNPopupItem* dnpopupItem;
...
@property (nonatomic, strong, readonly) __kindof UIView* dnviewForPopupInteractionGestureRecognizer;
...
@property (nullable, nonatomic, weak, readonly) __kindof UIViewController* dnpopupPresentationContainerViewController;
...
- (BOOL)dnpositionPopupCloseButton:(DNPopupCloseButton*)popupCloseButton;
...
- (void)dnviewWillMoveToPopupContainerContentView:(nullable DNPopupContentView*)popupContentView NS_REQUIRES_SUPER;

/// Called to notify the view controller that its view has just been added to the container controller's popup content view.
///
/// - Parameter popupContentView: The popup content view, or `nil`.
- (void)dnviewDidMoveToPopupContainerContentView:(nullable DNPopupContentView*)popupContentView NS_REQUIRES_SUPER;

@end

NS_SWIFT_UI_ACTOR
/// A set of methods, used to respond to popup presentation changes.
@protocol DNPopupPresentationDelegate <NSObject>

@optional

/// Notifies the delegate that the popup bar is about to be presented.
- (void)dnpopupPresentationControllerWillPresentPopupBar:(UIViewController*)popupPresentationController animated:(BOOL)animated;

/// Notifies the delegate that the popup bar has been presented.
- (void)dnpopupPresentationControllerDidPresentPopupBar:(UIViewController*)popupPresentationController animated:(BOOL)animated;

/// Notifies the delegate that the popup bar is about to be dismissed.
- (void)dnpopupPresentationControllerWillDismissPopupBar:(UIViewController*)popupPresentationController animated:(BOOL)animated;

/// Notifies the delegate that the popup bar has been dismissed.
- (void)dnpopupPresentationControllerDidDismissPopupBar:(UIViewController*)popupPresentationController animated:(BOOL)animated;

/// Notifies the delegate that the popup is about to be opened with the specified popup content controller.
- (void)dnpopupPresentationController:(UIViewController*)popupPresentationController willOpenPopupWithContentController:(UIViewController*)popupContentController animated:(BOOL)animated;

/// Notifies the delegate that the popup has been opened with the specified popup content controller.
- (void)dnpopupPresentationController:(UIViewController*)popupPresentationController didOpenPopupWithContentController:(UIViewController*)popupContentController  animated:(BOOL)animated;

/// Notifies the delegate that the popup is about to be closed with the specified popup content controller.
- (void)dnpopupPresentationController:(UIViewController*)popupPresentationController willClosePopupWithContentController:(UIViewController*)popupContentController animated:(BOOL)animated;

/// Notifies the delegate that the popup has been closed with the specified popup content controller.
- (void)dnpopupPresentationController:(UIViewController*)popupPresentationController didClosePopupWithContentController:(UIViewController*)popupContentController animated:(BOOL)animated;

/// Notifies the delegate that the popup is about to be opened.
- (void)dnpopupPresentationControllerWillOpenPopup:(UIViewController*)popupPresentationController animated:(BOOL)animated DN_DEPRECATED_API("Use popupPresentationController:willOpenPopupWithContentController:animated: instead");

/// Notifies the delegate that the popup has been opened.
- (void)dnpopupPresentationControllerDidOpenPopup:(UIViewController*)popupPresentationController animated:(BOOL)animated DN_DEPRECATED_API("Use popupPresentationController:didOpenPopupWithContentController:animated: instead");

/// Notifies the delegate that the popup is about to be closed.
- (void)dnpopupPresentationControllerWillClosePopup:(UIViewController*)popupPresentationController animated:(BOOL)animated DN_DEPRECATED_API("Use popupPresentationController:willClosePopupWithContentController:animated: instead");

/// Notifies the delegate that the popup has been closed.
- (void)dnpopupPresentationControllerDidClosePopup:(UIViewController*)popupPresentationController animated:(BOOL)animated DN_DEPRECATED_API("Use popupPresentationController:didClosePopupWithContentController:animated: instead");

@end

/// Popup presentation support for ``UIViewController`` subclasses.
@interface UIViewController (DNPopupPresentation)
...
- (void)dnpresentPopupBarWithContentViewController:(UIViewController*)controller animated:(BOOL)animated completion:(nullable void(^)(void))completion NS_REFINED_FOR_SWIFT NS_SWIFT_DISABLE_ASYNC;
...
///   - animated: Pass `true` to animate the presentation; otherwise, pass `false`.
///   - completion: The block to execute after the presentation finishes. This block has no return value and takes no parameters. You may specify `nil` for this parameter.
- (void)dnpresentPopupBarWithContentViewController:(UIViewController*)controller dnopenPopup:(BOOL)openPopup animated:(BOOL)animated completion:(nullable void(^)(void))completion NS_REFINED_FOR_SWIFT NS_SWIFT_DISABLE_ASYNC;
...
- (void)dnopenPopupAnimated:(BOOL)animated completion:(nullable void(^)(void))completion NS_REFINED_FOR_SWIFT NS_SWIFT_DISABLE_ASYNC;
...
- (void)dnclosePopupAnimated:(BOOL)animated completion:(nullable void(^)(void))completion NS_REFINED_FOR_SWIFT NS_SWIFT_DISABLE_ASYNC;
...
- (void)dndismissPopupBarAnimated:(BOOL)animated completion:(nullable void(^)(void))completion NS_REFINED_FOR_SWIFT NS_SWIFT_DISABLE_ASYNC;

/// The popup bar interaction style.
@property (nonatomic, assign) DNPopupInteractionStyle dnpopupInteractionStyle NS_REFINED_FOR_SWIFT;


/// The percent of the container controller's view height to drag before closing the popup.
@property (nonatomic, assign) double dnpopupSnapPercent NS_REFINED_FOR_SWIFT;


/// The popup bar managed by the system. (read-only)
@property (nonatomic, strong, readonly) DNPopupBar* dnpopupBar;
...
@property (nonatomic, assign) BOOL dnshouldExtendPopupBarUnderSafeArea;
...
///
/// See `DNPopupBar.inheritsAppearanceFromDockingView` and `DNPopupBar.customBarViewController` for more information.
- (void)setNeedsPopupBarAppearanceUpdate;

/// The popup content container view. (read-only)
@property (nonatomic, strong, readonly) DNPopupContentView* dnpopupContentView;

/// The state of the popup presentation. (read-only)
///
/// This property is KVO-compliant.
@property (nonatomic, readonly) DNPopupPresentationState dnpopupPresentationState;

/// The delegate that handles popup presentation-related messages.
@property (nonatomic, weak) id<DNPopupPresentationDelegate> dnpopupPresentationDelegate;

/// The content view controller of the receiver. If there is no popup presentation, the property will be @c nil. (read-only)
@property (nullable, nonatomic, strong, readonly) __kindof UIViewController* dnpopupContentViewController;
...
@interface UIViewController (DNPopupCustomContainer)
...
@property (nullable, nonatomic, strong, readonly) __kindof UIView* dnbottomDockingViewForPopupBar;
...
@property (nonatomic, assign, readonly) BOOL dnshouldFadePopupBarOnDismiss;
...
///
/// A default implementation is provided for @c UIViewController, @c UINavigationController and @c UITabBarController.
@property (nonatomic, readonly) CGRect dndefaultFrameForBottomDockingView;
...
@property (nonatomic, readonly) UIEdgeInsets dninsetsForBottomDockingView;
...
@property (nullable, nonatomic, strong, readonly) __kindof UIView* bottomDockingViewForPopup DN_UNAVAILABLE_API("Use bottomDockingViewForPopupBar instead.");
...
///
/// If the popup bar's @c inheritsAppearanceFromDockingView property is set to @c false, or a custom popup bar view controller is used, this method has no effect. See @c DNPopupBar.inheritsAppearanceFromDockingView and @c DNPopupBar.customBarViewController for more information.
- (void)dnupdatePopupBarAppearance DN_UNAVAILABLE_API("Use setNeedsPopupBarAppearanceUpdate instead.");
